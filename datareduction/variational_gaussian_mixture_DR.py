...
class VariationalGaussianMixture_DR(RandomVariable):
    ...
    def _clusterSSProyection(self, X, n_clusters, transformer=None, X_proyected=None):
        ...
        centerProyected = transformer.transform(centerJoin)
        kmeans = MiniBatchKMeans(n_clusters=n_clusters, init=centerProyected).fit(X_proyected)
        weights = np.asarray([sum(kmeans.labels_ == x) for x in range(0, n_clusters)])
        self.clusterError = self._clusteringError(X_proyected,kmeans)

        finalClusters = np.zeros((n_clusters, centerJoin.shape[1]))
        for i in range(0, n_clusters):
            ...
            finalClusters[i, :] = np.mean(XCluster_Join, axis=0)

        D=X.shape[1]
        self.X_dr = {'X': finalClusters[:, 0:D], 'XX': finalClusters[:, D:D + D * D].reshape((n_clusters, D, D)),
                'W': weights}
    ...
    def fit(self, X, iter_max=100, n_clusters=10, cluster_method="SS", transformer=None, X_proyected=None):
        ...
        elif cluster_method == "SSProyection":
            self._clusterSSProyection(X,n_clusters,transformer,X_proyected)
    ...
    def _variational_maximization_DR(self, X_dr, r):
        ...
        self.component_size = self.component_size + 0.1
