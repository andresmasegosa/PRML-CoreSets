...
test_ll = np.zeros([samples.shape[0], 25])
time = np.zeros([samples.shape[0], 25])
...
for m in range(0,samples.shape[0]):
    ...
    test_ll[m], time[m] = bpca_svi.fit(x_train, initial="eigen", iter_max=25, batch_size=samples[m], testset=x_test)

    text = "PCA-SVI-Time " + str(m)
    sc.api_call(
        "chat.postMessage",
...
np.savetxt('./figs/PCA_GPS_SVI-Time-LL.txt',test_ll)
np.savetxt('./figs/PCA_GPS_SVI-Time-Time.txt',time)
